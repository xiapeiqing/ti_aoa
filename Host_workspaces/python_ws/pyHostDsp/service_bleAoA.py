...
MQTT_NAME = "/m2cambot/MEAS/BleAoARawDatRpt"
...
index = 0
try:
    mqproxy.publish(mqttutils.MQTT_CMD_LOGBLEAOARAW,'rawdatalog')
    while True:
        msg = None
        now = time.time()
        ...
        while not mqueue.empty():
            ...
            if msg is not None:
                ...
                #mqproxy.publish(mqttutils.MQTT_CMD_LOGBLEAOARAW,datfilename) # start the new cpp data collection
                if True:
                    # write to file and call matlab to process
                    with open('datapkt{}.bin'.format(index),'wb') as newfile:
                        newfile.write(values);
                        index += 1
                else:
                    # python real itme processing
                    IQ0index = 7
